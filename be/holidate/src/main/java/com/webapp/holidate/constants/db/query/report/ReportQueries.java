...
public class ReportQueries {
    ...
    // ========== Admin Revenue Report Queries ==========

    // Query to get daily system revenue (from SystemDailyReport)
    public static final String GET_SYSTEM_DAILY_REVENUE = "SELECT " +
            "  sdr.report_date AS period, " +
            "  sdr.gross_revenue AS revenue " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
            "WHERE sdr.report_date >= :fromDate " +
            "AND sdr.report_date <= :toDate " +
            "ORDER BY sdr.report_date ASC";

    // Query to get weekly system revenue (from SystemDailyReport)
    public static final String GET_SYSTEM_WEEKLY_REVENUE = "SELECT " +
            "  DATE_SUB(sdr.report_date, INTERVAL WEEKDAY(sdr.report_date) DAY) AS period, " +
            "  SUM(sdr.gross_revenue) AS revenue " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
            "WHERE sdr.report_date >= :fromDate " +
            "AND sdr.report_date <= :toDate " +
            "GROUP BY DATE_SUB(sdr.report_date, INTERVAL WEEKDAY(sdr.report_date) DAY) " +
            "ORDER BY period ASC";

    // Query to get monthly system revenue (from SystemDailyReport)
    public static final String GET_SYSTEM_MONTHLY_REVENUE = "SELECT " +
            "  DATE_FORMAT(sdr.report_date, '%Y-%m-01') AS period, " +
            "  SUM(sdr.gross_revenue) AS revenue " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
            "WHERE sdr.report_date >= :fromDate " +
            "AND sdr.report_date <= :toDate " +
            "GROUP BY DATE_FORMAT(sdr.report_date, '%Y-%m-01') " +
            "ORDER BY period ASC";

    // Query to get total system revenue for summary
    public static final String GET_SYSTEM_TOTAL_REVENUE = "SELECT " +
            "  COALESCE(SUM(sdr.gross_revenue), 0) AS totalRevenue " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
            "WHERE sdr.report_date >= :fromDate " +
            "AND sdr.report_date <= :toDate";

    // Query to get revenue breakdown by hotel
    public static final String GET_REVENUE_BY_HOTEL = "SELECT " +
            "  h.id AS filterId, " +
            "  h.name AS filterName, " +
            "  COALESCE(SUM(hdr.total_revenue), 0) AS revenue " +
            "FROM " + DbTableNames.HOTEL_DAILY_REPORTS + " hdr " +
            "INNER JOIN " + DbTableNames.HOTELS + " h ON hdr.hotel_id = h.id " +
            "WHERE hdr.report_date >= :fromDate " +
            "AND hdr.report_date <= :toDate " +
            "GROUP BY h.id, h.name " +
            "ORDER BY revenue DESC";

    // Query to get revenue breakdown by city
    public static final String GET_REVENUE_BY_CITY = "SELECT " +
            "  c.id AS filterId, " +
            "  c.name AS filterName, " +
            "  COALESCE(SUM(hdr.total_revenue), 0) AS revenue " +
            "FROM " + DbTableNames.HOTEL_DAILY_REPORTS + " hdr " +
            "INNER JOIN " + DbTableNames.HOTELS + " h ON hdr.hotel_id = h.id " +
            "INNER JOIN " + DbTableNames.CITIES + " c ON h.city_id = c.id " +
            "WHERE hdr.report_date >= :fromDate " +
            "AND hdr.report_date <= :toDate " +
            "GROUP BY c.id, c.name " +
            "ORDER BY revenue DESC";

    // Query to get revenue breakdown by province
    public static final String GET_REVENUE_BY_PROVINCE = "SELECT " +
            "  p.id AS filterId, " +
            "  p.name AS filterName, " +
            "  COALESCE(SUM(hdr.total_revenue), 0) AS revenue " +
            "FROM " + DbTableNames.HOTEL_DAILY_REPORTS + " hdr " +
            "INNER JOIN " + DbTableNames.HOTELS + " h ON hdr.hotel_id = h.id " +
            "INNER JOIN " + DbTableNames.PROVINCES + " p ON h.province_id = p.id " +
            "WHERE hdr.report_date >= :fromDate " +
            "AND hdr.report_date <= :toDate " +
            "GROUP BY p.id, p.name " +
            "ORDER BY revenue DESC";

    // ========== Admin Hotel Performance Report Queries ==========

    // Base query for hotel performance summary (CTE for MySQL 8.0+)
    // Note: MySQL 8.0+ supports CTE. For older versions, we can use subquery.
    public static final String GET_HOTEL_PERFORMANCE_BASE = "WITH HotelPerformanceSummary AS (" +
            "  SELECT " +
            "    hdr.hotel_id AS hotelId, " +
            "    COALESCE(SUM(hdr.total_revenue), 0) AS totalRevenue, " +
            "    COALESCE(SUM(hdr.completed_bookings), 0) AS totalCompletedBookings, " +
            "    COALESCE(SUM(hdr.cancelled_bookings), 0) AS totalCancelledBookings, " +
            "    COALESCE(SUM(hdr.created_bookings), 0) AS totalCreatedBookings, " +
            "    COALESCE(SUM(hdr.occupied_room_nights), 0) AS sumOccupiedRoomNights, " +
            "    COALESCE(SUM(hdr.total_room_nights), 0) AS sumTotalRoomNights " +
            "  FROM " + DbTableNames.HOTEL_DAILY_REPORTS + " hdr " +
            "  WHERE hdr.report_date >= :fromDate " +
            "  AND hdr.report_date <= :toDate " +
            "  GROUP BY hdr.hotel_id" +
            ") " +
            "SELECT " +
            "  h.id AS hotelId, " +
            "  h.name AS hotelName, " +
            ...
            "  s.totalCreatedBookings, " +
            "  s.totalCancelledBookings, " +
            "  CASE " +
            "    WHEN s.sumTotalRoomNights > 0 THEN (s.sumOccupiedRoomNights * 100.0 / s.sumTotalRoomNights) " +
            "    ELSE 0.0 " +
            "  END AS averageOccupancyRate, " +
            "  CASE " +
            "    WHEN s.totalCreatedBookings > 0 THEN (s.totalCancelledBookings * 100.0 / s.totalCreatedBookings) " +
            "    ELSE 0.0 " +
            "  END AS cancellationRate " +
            "FROM HotelPerformanceSummary s " +
            "INNER JOIN " + DbTableNames.HOTELS + " h ON s.hotelId = h.id " +
            "WHERE (:cityId IS NULL OR h.city_id = :cityId) " +
            "AND (:provinceId IS NULL OR h.province_id = :provinceId)";

    // Query to count total hotels for pagination
    public static final String COUNT_HOTEL_PERFORMANCE = "WITH HotelPerformanceSummary AS (" +
            "  SELECT " +
            "    hdr.hotel_id AS hotelId " +
            "  FROM " + DbTableNames.HOTEL_DAILY_REPORTS + " hdr " +
            "  WHERE hdr.report_date >= :fromDate " +
    ...
    // ========== Admin Users Summary Report Queries ==========
    
    // Query to get new users growth from SystemDailyReport
    public static final String GET_NEW_USERS_GROWTH = "SELECT " +
            "  COALESCE(SUM(sdr.new_customers_registered), 0) AS newCustomers, " +
            "  COALESCE(SUM(sdr.new_partners_registered), 0) AS newPartners " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
            "WHERE sdr.report_date >= :fromDate " +
            "AND sdr.report_date <= :toDate";

    // Query to get total user counts by role from User and Role tables
    public static final String GET_TOTAL_USER_COUNTS = "SELECT " +
            "  r.name AS roleName, " +
            "  COUNT(u.id) AS userCount " +
            "FROM " + DbTableNames.USERS + " u " +
            "INNER JOIN " + DbTableNames.ROLES + " r ON u.role_id = r.id " +
            "WHERE r.name IN (:customerRoleName, :partnerRoleName) " +
            "GROUP BY r.name";

    // ========== Admin Trend Analysis Report Queries ==========
    
    // Query to get seasonality data (monthly aggregation from SystemDailyReport)
    public static final String GET_SEASONALITY_DATA = "SELECT " +
            "  DATE_FORMAT(sdr.report_date, '%Y-%m-01') AS month, " +
            "  COALESCE(SUM(sdr.gross_revenue), 0) AS totalRevenue, " +
            "  COALESCE(SUM(sdr.total_bookings_created), 0) AS totalBookings " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
            "WHERE sdr.report_date >= :fromDate " +
            "AND sdr.report_date <= :toDate " +
            "GROUP BY DATE_FORMAT(sdr.report_date, '%Y-%m-01') " +
    ...
    // Query to get popular locations by city
    public static final String GET_POPULAR_LOCATIONS_BY_CITY = "SELECT " +
            "  c.id AS locationId, " +
            "  c.name AS locationName, " +
            "  COALESCE(SUM(hdr.total_revenue), 0) AS totalRevenue, " +
            "  COALESCE(SUM(hdr.completed_bookings), 0) AS totalBookings " +
            "FROM " + DbTableNames.HOTEL_DAILY_REPORTS + " hdr " +
            "INNER JOIN " + DbTableNames.HOTELS + " h ON hdr.hotel_id = h.id " +
            "INNER JOIN " + DbTableNames.CITIES + " c ON h.city_id = c.id " +
            "WHERE hdr.report_date >= :fromDate " +
    ...
    // Query to get popular locations by province
    public static final String GET_POPULAR_LOCATIONS_BY_PROVINCE = "SELECT " +
            "  p.id AS locationId, " +
    ...
    // Query to get popular room types by view
    public static final String GET_POPULAR_ROOM_TYPES_BY_VIEW = "SELECT " +
            "  r.view AS roomCategory, " +
            "  COALESCE(SUM(rdp.booked_room_nights), 0) AS totalBookedNights " +
            "FROM " + DbTableNames.ROOM_DAILY_PERFORMANCES + " rdp " +
            "INNER JOIN " + DbTableNames.ROOMS + " r ON rdp.room_id = r.id " +
            "WHERE rdp.report_date >= :fromDate " +
            "AND rdp.report_date <= :toDate " +
            "GROUP BY r.view " +
            "ORDER BY totalBookedNights DESC " +
            "LIMIT :limit";

    // Query to get popular room types by bed type
    public static final String GET_POPULAR_ROOM_TYPES_BY_BED_TYPE = "SELECT " +
            "  bt.name AS roomCategory, " +
            "  COALESCE(SUM(rdp.booked_room_nights), 0) AS totalBookedNights " +
            "FROM " + DbTableNames.ROOM_DAILY_PERFORMANCES + " rdp " +
            "INNER JOIN " + DbTableNames.ROOMS + " r ON rdp.room_id = r.id " +
            "INNER JOIN " + DbTableNames.BED_TYPES + " bt ON r.bed_type_id = bt.id " +
            "WHERE rdp.report_date >= :fromDate " +
            "AND rdp.report_date <= :toDate " +
    ...
    // Query to get popular room types by occupancy (maxAdults + maxChildren)
    public static final String GET_POPULAR_ROOM_TYPES_BY_OCCUPANCY = "SELECT " +
            "  CONCAT(r.max_adults, ' Adults, ', r.max_children, ' Children') AS roomCategory, " +
            "  COALESCE(SUM(rdp.booked_room_nights), 0) AS totalBookedNights " +
            "FROM " + DbTableNames.ROOM_DAILY_PERFORMANCES + " rdp " +
            "INNER JOIN " + DbTableNames.ROOMS + " r ON rdp.room_id = r.id " +
            "WHERE rdp.report_date >= :fromDate " +
            "AND rdp.report_date <= :toDate " +
            "GROUP BY r.max_adults, r.max_children " +
            "ORDER BY totalBookedNights DESC " +
            "LIMIT :limit";

    // ========== Admin Financials Report Queries ==========
    
    // Query to get daily financials data (from SystemDailyReport)
    public static final String GET_FINANCIALS_DAILY_DATA = "SELECT " +
            "  sdr.report_date AS period, " +
            "  sdr.gross_revenue AS grossRevenue, " +
            "  sdr.net_revenue AS netRevenue " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
            "WHERE sdr.report_date >= :fromDate " +
            "AND sdr.report_date <= :toDate " +
            "ORDER BY sdr.report_date ASC";

    // Query to get weekly financials data (from SystemDailyReport)
    public static final String GET_FINANCIALS_WEEKLY_DATA = "SELECT " +
            "  DATE_SUB(sdr.report_date, INTERVAL WEEKDAY(sdr.report_date) DAY) AS period, " +
            "  SUM(sdr.gross_revenue) AS grossRevenue, " +
            "  SUM(sdr.net_revenue) AS netRevenue " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
            "WHERE sdr.report_date >= :fromDate " +
            "AND sdr.report_date <= :toDate " +
            "GROUP BY DATE_SUB(sdr.report_date, INTERVAL WEEKDAY(sdr.report_date) DAY) " +
    ...
    // Query to get monthly financials data (from SystemDailyReport)
    public static final String GET_FINANCIALS_MONTHLY_DATA = "SELECT " +
            "  DATE_FORMAT(sdr.report_date, '%Y-%m-01') AS period, " +
            "  SUM(sdr.gross_revenue) AS grossRevenue, " +
            "  SUM(sdr.net_revenue) AS netRevenue " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
    ...
    // Query to get total financials for summary
    public static final String GET_FINANCIALS_TOTAL = "SELECT " +
            "  COALESCE(SUM(sdr.gross_revenue), 0) AS totalGrossRevenue, " +
            "  COALESCE(SUM(sdr.net_revenue), 0) AS totalNetRevenue " +
            "FROM " + DbTableNames.SYSTEM_DAILY_REPORTS + " sdr " +
            "WHERE sdr.report_date >= :fromDate " +
            "AND sdr.report_date <= :toDate";
}
