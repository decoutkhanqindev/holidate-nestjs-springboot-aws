...
- **pendingPaymentBookings:** `int` (not null, default=0)

  **Mô tả nghiệp vụ:** Tổng số booking có trạng thái PENDING_PAYMENT được tạo trong ngày (dựa vào createdAt và status = 'PENDING_PAYMENT').

- **confirmedBookings:** `int` (not null, default=0)

  **Mô tả nghiệp vụ:** Tổng số booking có trạng thái CONFIRMED được tạo trong ngày (dựa vào createdAt và status = 'CONFIRMED').

- **checkedInBookings:** `int` (not null, default=0)

  **Mô tả nghiệp vụ:** Tổng số booking có trạng thái CHECKED_IN được tạo trong ngày (dựa vào createdAt và status = 'CHECKED_IN').

- **completedBookings:** `int` (not null, default=0)

  **Mô tả nghiệp vụ:** Tổng số booking được hoàn thành trong ngày (dựa vào checkOutDate = D và status = 'COMPLETED'). Khác với các field trạng thái khác, field này đếm theo checkOutDate chứ không phải createdAt.
...
- **rescheduledBookings:** `int` (not null, default=0)

  **Mô tả nghiệp vụ:** Tổng số booking có trạng thái RESCHEDULED được tạo trong ngày (dựa vào createdAt và status = 'RESCHEDULED').
...
1. **Lấy danh sách khách sạn:** Lấy tất cả hotelId đang hoạt động.

2. **Lặp qua từng khách sạn (H):**
   ...
   - Lặp qua từng booking:
     - Nếu createdAt trong ngày D:
       - `createdBookings++`
       - Đếm booking theo trạng thái tại thời điểm tạo (status tại createdAt):
         - Nếu status = 'PENDING_PAYMENT': `pendingPaymentBookings++`
         - Nếu status = 'CONFIRMED': `confirmedBookings++`
         - Nếu status = 'CHECKED_IN': `checkedInBookings++`
         - Nếu status = 'RESCHEDULED': `rescheduledBookings++`
...
**Logic Nghiệp vụ API:** API tổng hợp các cột createdBookings, pendingPaymentBookings, confirmedBookings, checkedInBookings, completedBookings, cancelledBookings, rescheduledBookings từ HotelDailyReport trong khoảng thời gian yêu cầu. Tỷ lệ hủy phòng (cancellationRate) được tính động tại thời điểm gọi API: `(SUM(cancelledBookings) / SUM(createdBookings)) * 100`.
...
1. Nhận 2 khoảng thời gian: currentPeriod và previousPeriod.
2. Gọi phương thức lấy dữ liệu báo cáo cơ sở (ví dụ getRevenueData(from, to)) hai lần.
3. Nhận hai bộ kết quả.
4. Tính toán các chỉ số so sánh (difference, percentageChange).
5. Xây dựng và trả về đối tượng JSON cuối cùng.
