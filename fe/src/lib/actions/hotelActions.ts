...
export async function updateHotelAction(id: string, formData: FormData) {
    try {
        // Kiểm tra xem có phải chỉ update venues/amenities/policy không (không có name)
        const name = formData.get('name') as string;
        const hasOnlyVenuesOrAmenities = !name && (
            formData.has('entertainmentVenuesWithDistanceToUpdate[0].entertainmentVenueId') ||
            formData.has('entertainmentVenuesWithDistanceToAdd[0].entertainmentVenueId') ||
            formData.has('entertainmentVenuesToAdd[0].name') ||
            formData.has('entertainmentVenueIdsToRemove') ||
            formData.has('amenityIdsToAdd[]') ||
            formData.has('policy.checkInTime')
        );

        // Chỉ validate name nếu không phải là update venues/amenities/policy riêng
        if (!hasOnlyVenuesOrAmenities && (!name || name.trim() === '')) {
            return { error: "Tên khách sạn là bắt buộc." };
        }
...
// ACTION ĐỂ TẠO KHÁCH SẠN MỚI (Dành cho Super Admin - không redirect, để modal tự xử lý)
export async function createHotelActionSuperAdmin(formData: FormData) {
    try {
        const name = formData.get('name') as string;
        ...
        console.log('[createHotelActionSuperAdmin] Creating hotel with location IDs:', {
            countryId, provinceId, cityId, districtId, wardId, streetId, partnerId
        });
        ...
        console.log('[createHotelActionSuperAdmin] Final payload before sending:', JSON.stringify(payload, null, 2));
