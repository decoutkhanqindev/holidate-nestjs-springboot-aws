...
export default function SearchPageClient({
    ...
    const [hotels, setHotels] = useState<HotelResponse[]>(initialHotels);
    const [amenityCategories, setAmenityCategories] = useState<AmenityCategory[]>(initialAmenityCategories);
    ...
    const [totalHotelsFound, setTotalHotelsFound] = useState(initialTotalItems);
    const [loading, setLoading] = useState(false); // Không loading nếu đã có initial data
    const [loadingAmenities, setLoadingAmenities] = useState(initialAmenityCategories.length === 0);
    ...
    // Load amenities nếu chưa có initial data
    useEffect(() => {
        if (initialAmenityCategories.length === 0) {
    ...
    }, [initialAmenityCategories.length]);

    // Fetch page 0 khi searchParams, selectedAmenities, hoặc priceRange thay đổi
    ...
    useEffect(() => {
        ...
        if (initialHotels.length > 0 && selectedAmenities.length === 0 && 
            ...
            const paramsMatch = Array.from(searchParams.entries()).every(([key, value]) => {
                return currentUrlParams.get(key) === value;
            });
            if (paramsMatch) {
                // Không cần fetch - dùng initial data
                return;
            }
        }
